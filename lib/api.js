...
const apiReference = require('./api-reference.json');
...
class Api {
    constructor(applicationUrl, apiResources = apiReference) {
    ...
    makeUri(route) {
        let base = this.applicationUrl.replace(/\/$/, '');
        let path = route.replace(/^\//, '').replace(/\/+/g, '/');
        return `${base}/${path}`;
    }
